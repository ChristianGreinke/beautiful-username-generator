...
(async ()=>{
    ...
    try {
        ...
        for( const file of files ) {
            ...
            if( fStat.isFile() && file.indexOf('flattenedAttributes')>=0) await unlink(fPath);
    ...
    const attributes = (await readFile('./raw/attributes.csv')).toString().split('\n').map(camelize);
    ...
    const writes = attributes.map(async (f)=>{
        ...
        await writeFile(`./api/flattenedAttributes_${maxLength}.bin`,Buffer.from(ab),{ flag: 'a+' });
    });
